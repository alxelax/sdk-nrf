	      ...
	      bool is_md_crc_correct, bool is_d_crc_correct, uint32_t fresh_cnt)
{
	static struct emds_snapshot_metadata metadata = {
		.marker = EMDS_SNAPSHOT_METADATA_MARKER,
	};
	uint8_t data[128] = {0};
	uint8_t data_len = rand() % 128 + sizeof(struct emds_data_entry) + 1;
	...
	zassert_ok(flash_area_write(partition->fa, partition->fa->fa_off + metadata_off, &metadata,
				    sizeof(metadata)),
...
{
	struct emds_snapshot_candidate candidate = {.partition_index = -1};

	zassert_ok(emds_flash_scan_partition(&partition[0], &candidate),
		   "Failed to scan partition 0");
	zassert_equal(candidate.partition_index, -1,
		      "Partition index has been changed for emtpty partition");
...
ZTEST(emds_flash, test_scanning_partitions_equality)
{
	...
	zassert_ok(emds_flash_scan_partition(&partition[0], &candidate),
		   "Failed to scan partition 0");
	zassert_equal(candidate.partition_index, 0, "Partition index mismatch for partition 0");
	...
	zassert_equal(candidate.metadata.fresh_cnt, 3, "Fresh count mismatch for partition 0");

	zassert_ok(emds_flash_scan_partition(&partition[1], &candidate),
		   "Failed to scan partition 1");
	zassert_equal(candidate.partition_index, 1, "Partition index mismatch for partition 1");
	zassert_equal(candidate.metadata_off,
		      partition[1].fa->fa_size -
			      CONFIG_EMDS_MAX_CANDIDATES * sizeof(struct emds_snapshot_metadata),
		      "Metadata offset mismatch for partition 1");
	zassert_equal(candidate.metadata.fresh_cnt, 6, "Fresh count mismatch for partition 1");
}
...
{
	struct emds_snapshot_candidate candidate;
	int partition_index = rand() % PARTITIONS_NUM_MAX;
	struct emds_snapshot_metadata *metadata = NULL;
...
ZTEST(emds_flash, test_scanning_partitions_freshingness)
{
	struct emds_snapshot_candidate candidate;
	int partition_index = rand() % PARTITIONS_NUM_MAX;
	struct emds_snapshot_metadata *metadata = NULL;
...
{
	struct emds_snapshot_candidate allocated_snapshot = {.partition_index = -1};
	int partition_index = rand() % PARTITIONS_NUM_MAX;
...
ZTEST(emds_flash, test_allocation_after_freshest_same_partition)
{
	...
	metadata = snapshot_make(&partition[partition_index], metadata, metadata_off, true, true,
				 fresh_cnt);
	zassert_not_null(metadata, "Failed to create snapshot on partition %d", partition_index);
	freshest_snapshot.partition_index = partition_index;
...
ZTEST(emds_flash, test_allocation_next_partition_if_freshest_full)
{
	...
	zassert_equal(rc, -ENOMEM,
		      "Expected -ENOMEM when trying to allocate snapshot on full partition %d",
		      partition_index);
}
...
ZTEST(emds_flash, test_allocation_if_metadata_garbaged)
{
	...
	zassert_ok(flash_area_write(fa, fa->fa_off + fa->fa_size - sizeof(garbage), &garbage,
				    sizeof(garbage)),
...
ZTEST(emds_flash, test_allocation_if_data_garbaged)
{
	...
	zassert_ok(flash_area_write(fa, fa->fa_off, &garbage, sizeof(garbage)),
		   "Failed to write garbage into metadata flash area");
...
ZTEST(emds_flash, test_write_speed)
{
	...
	zassert_true(store_time_us < EXPECTED_STORE_TIME_1024, "Storing %d bytes took to long time",
		     sizeof(data_in_big));
