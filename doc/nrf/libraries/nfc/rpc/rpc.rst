...
The :ref:`NFC <ug_nfc>` Remote Procedure Call (RPC) solution is a set of libraries that allows using the NFC running entirely on a separate device or CPU.
...
This is accomplished by running a full NFC functionality on one device and serializing the API from another device.
Use this solution when you do not want your firmware to include the NFC, for example to offload the application CPU, save memory, or to be able to build your application in a different environment.
