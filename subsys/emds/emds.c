...
int emds_load(void)
{
	...
	LOG_DBG("Found freshest snapshot in partition %d with fresh_cnt %u",
		freshest_snapshot.index, freshest_snapshot.metadata.fresh_cnt);

	return emds_read_data(partition[freshest_snapshot.index].fa, &freshest_snapshot.metadata);
}
...
int emds_store(void)
{
	...
	int idx = allocated_snapshot.index;
	int rc = 0;
